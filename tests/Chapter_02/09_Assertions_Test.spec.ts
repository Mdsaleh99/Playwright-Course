...
// Assertions means comparing actual value to expected value, use expect
// 2 types of assertions - hard assertions and soft assertions
...
test("Hard Assertions in playwright", async ({ page }) => {
    ...
    await expect(page.locator('span[id="title"]').first()).toHaveText(
        "Latest posts from Testers Talk"
    );
